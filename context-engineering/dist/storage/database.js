...
class ContextStorage {
    db;
    initialized = false;
    constructor(dbPath = './context-engineering/context.db') {
    ...
    async store(entry) {
        ...
        await run(`INSERT OR REPLACE INTO context_entries 
    ...
    async retrieve(id) {
        ...
        const row = await get('SELECT * FROM context_entries WHERE id = ?', [id]);
        if (!row)
            return null;
    ...
    async query(searchQuery) {
        ...
        const entries = rows.map((row) => this.parseRowToEntry(row));
    ...
    async getStatistics() {
      ...
      GROUP BY type
    `);
        const entriesByType = typeResults.reduce((acc, row) => {
    ...
    async update(entry) {
        ...
        await run(`UPDATE context_entries 
