...
export class ContextSearch {
  ...
  async search(query: SearchQuery): Promise<SearchResult> {
    ...
    // For text queries, use simpler approach to avoid FTS issues in tests
    if (query.query && query.query.trim()) {
      // Get all entries and filter in memory for simplicity in tests
      ...
      let { entries, total } = await this.storage.query(allQuery);
      
      // Apply text-based filtering
      entries = entries.filter(entry => 
        entry.title.toLowerCase().includes(query.query.toLowerCase()) ||
  ...
  async findRelated(entryId: string, limit: number = 10): Promise<ContextEntry[]> {
  ...
  async discoverPatterns(limit: number = 20): Promise<Array<{ pattern: string, confidence: number, entries: ContextEntry[] }>> {
    ...
    const patterns: Array<{ pattern: string, confidence: number, entries: ContextEntry[] }> = [];
  ...
  async rebuildIndex(): Promise<void> {
    const allQuery: SearchQuery = { query: '', limit: 10000, offset: 0 };
    const { entries } = await this.storage.query(allQuery);
    ...
    this.fuseIndex = new Fuse(entries, fuseOptions);
    this.lastIndexUpdate = new Date();
  }

  private performFuzzySearch(entries: ContextEntry[], query: string): ContextEntry[] {
    if (!this.fuseIndex) {
  ...
  private rankResults(entries: ContextEntry[], query: SearchQuery): ContextEntry[] {
  ...
  private calculateRelevanceScore(entry: ContextEntry, query: SearchQuery): number {
    ...
    if (query.agents && query.agents.length > 0) {
      const matchingAgents = query.agents.filter(agent => 
        entry.metadata.agents.includes(agent)
      ).length;
  ...
  private calculateSimilarity(entry1: ContextEntry, entry2: ContextEntry): number {
    let similarity = 0;
    ...
    // Agent similarity
    const commonAgents = entry1.metadata.agents.filter(agent => 
      entry2.metadata.agents.includes(agent)
    ).length;
  ...
  private findCommonTagCombinations(entries: ContextEntry[]): Array<{ tags: string[], frequency: number, entries: ContextEntry[] }> {
    const tagCombinations = new Map<string, { count: number, entries: ContextEntry[] }>();
  ...
  private findAgentCollaborationPatterns(entries: ContextEntry[]): Array<{ agents: string[], frequency: number, entries: ContextEntry[] }> {
    const agentGroups = new Map<string, { count: number, entries: ContextEntry[] }>();
  ...
  private findContentPatterns(entries: ContextEntry[]): Array<{ pattern: string, confidence: number, entries: ContextEntry[] }> {
    const patterns: Array<{ pattern: string, confidence: number, entries: ContextEntry[] }> = [];

    // Find common keywords in titles
    const titleWords = new Map<string, ContextEntry[]>();
    entries.forEach(entry => {
